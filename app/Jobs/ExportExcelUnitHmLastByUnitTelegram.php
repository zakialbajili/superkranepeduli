...
class ExportExcelUnitHmLastByUnitTelegram implements ShouldQueue
{
        ...
        try {
            ...
            $data = DB::table('tunitloghmtelegram')
                ...
                ->select([
                    ...
                    DB::raw('max(tunitloghmtelegram.hm_crane) as HM_Utama_HM_Atas'),
                    DB::raw('max(tunitloghmtelegram.hm_cabin) as HM_Tambahan_HM_Bawah')
                ])
                ->groupBy([
                    'tunitloghmtelegram.reg_date',
                    'tunit.unit_no',
                    'tunit.unit_no_alternate_1',
                    'tunit.unit_no_alternate_2',
                    'tunitmanufacture.name',
                    'tunitmodel.name',
                ])
            ...
            $filename = 'UnitHMTelegramPerDayByUnit_' . $this->user->employees->employee_no . '_' . $now . '.xlsx';
