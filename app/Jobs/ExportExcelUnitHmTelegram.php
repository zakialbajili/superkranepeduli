...
class ExportExcelUnitHmTelegram implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $data = DB::table('tunitloghmtelegram')
                ->join('tunit', 'tunitloghmtelegram.fk_unit_id', '=', 'tunit.pk_unit_id')
                ...
                ->select([
                    'tunitloghmtelegram.reg_date as Tanggal',
                    ...
                    'tunitloghmtelegram.hm_crane as HM Utama / HM Atas',
                    'tunitloghmtelegram.hm_cabin as HM Tambahan / HM Bawah',
                    'tunitloghmtelegram.created_date as Tanggal Disubmit',
                    'tunitloghmtelegram.created_by as NIK Disubmit Oleh',
                    'tunitloghmtelegram.created_by_name as Nama Disubmit Oleh',
                    'tunitloghmtelegram.created_by_position as Posisi Disubmit Oleh',
                ])
                ->where(function ($query) use ($datafilter) {
                    if ($datafilter["transactiondate"] != "") {
                        $datadate = explode(' - ', $datafilter["transactiondate"]);
                        if (count($datadate) > 0) {
                            $query->Where('tunitloghmtelegram.reg_date', ">=", $datadate[0]);
                            $query->Where('tunitloghmtelegram.reg_date', "<=", $datadate[1]);
                        }
                    }
                })
                ->get();
