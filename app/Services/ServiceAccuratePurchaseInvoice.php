...
class ServiceAccuratePurchaseInvoice
{
    public function insertToApp($event)
    {
        try {
            ...
            $response = Http::withToken((config('app.ACC_TOKEN')))
                ...
                ])->get($host . '/accurate/api/purchase-invoice/detail.do', ["id" => $id]);
            ...
            $aolprheader = [
                ...
                'purchaseAmount' => $responsebody['purchaseAmount'],
            ...
            $data = DB::table('taolpurchaseinvoiceheader')
                ->where("id", $id)
                ->first();
            if ($data) {
                DB::table('taolpurchaseinvoiceheader')
                    ->where("id", $id)
                    ->update($aolprheader);
            } else {
                DB::table('taolpurchaseinvoiceheader')
            ...
            foreach ($responsebody['detailItem'] as $detailItem) {
                ...
                $aolprdetail = [
                    'id' => $detailItem['id'],
                    'purchaseInvoiceId' => $detailItem['purchaseInvoiceId'],
                ...
                $aolitem = DB::table('taolpurchaseinvoicedetail')
                    ->where("id", $detailItem['id'])
                    ->where("purchaseInvoiceId", $id)
                    ->first();
                if ($aolitem) {
                    DB::table('taolpurchaseinvoicedetail')
                        ->where("id", $detailItem['id'])
                        ->where("purchaseInvoiceId", $id)
                        ->update($aolprdetail);
                } else {
